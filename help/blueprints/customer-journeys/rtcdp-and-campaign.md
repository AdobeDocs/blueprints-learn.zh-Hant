...
展示Adobe[!DNL Experience Platform]及其即時客戶設定檔和集中式細分工具如何與Adobe[!DNL Campaign]搭配使用，以提供個人化的交談。
...
* 建議將Experience Platform和[!DNL Campaign]布建在相同的IMS組織中，並使用Adobe Admin Console進行使用者存取。 這也可確保客戶可以從行銷 UI 中使用解決方案切換器
...
* Adobe[!DNL Campaign]是所有使用中設定檔的真實來源，表示設定檔必須存在於Adobe[!DNL Campaign]中，且不應根據Experience Platform區段建立新設定檔。
...
1. 設定[!DNL Azure] Blob儲存體目的地以搭配Adobe[!DNL Campaign]使用。
...
>[!IMPORTANT]
>
>此時瞭解設定檔和事件資料的Experience Platform中資料模型非常重要，這樣您才能知道Adobe[!DNL Campaign]中需要哪些資料。
...
* Experience Platform Mobile SDK 路由：
   * 運用Adobe標籤和[!DNL Campaign Classic]擴充功能來設定您與Experience Platform Mobile SDK的整合
...
>[!IMPORTANT]
>
>如果您部署[!DNL Campaign] SDK並使用其他Experience Cloud應用程式，他們將需要使用Experience Platform行動SDK來收集資料。 這會在裝置上建立重複的用戶端呼叫。
