...
若要將方案用於Real-time Customer Profile，您必須先確保其已正確設定。 此步驟表示將您在LID實驗室中識別的內容當成主要/個人身分、關係身分等，並確保針對每個結構描述進行這些設定。 完成所有工作後，您即可啟用結構描述以用於設定檔。
...
每個結構描述都需要主要身分欄位，才能與即時客戶個人檔案搭配使用。 若要將欄位標示為主要身分，請遵循下列步驟。
...
>[!NOTE]
>
>另請注意，您現在會在左側邊欄中看到下列專案。 身分（主要或非主要）會顯示在這裡，而&#x200B;**主要**&#x200B;身分也標示為必填欄位。
>
>
>
...
每個結構描述都可以&#x200B;**選擇性地包含**&#x200B;其他人員身分欄位。 此規則適用於與Real-Time Customer Profile搭配使用的任何結構。 若要將欄位標示為人員身分，請針對您先前建立的「客戶帳戶」綱要執行下列動作。
...
若要如ERD所述，將「計畫」結構描述與客戶帳戶結構描述建立關聯，您必須定義關係。 若要在客戶帳戶與計畫（查詢）結構描述之間建立結構描述關係，請遵循下列步驟。
...
1. 在「參考結構描述」選取方塊中，選擇名為&#x200B;**dep： Plan \[Lookup]**&#x200B;的結構描述（這是為您預先建立的結構描述）
1. 按一下&#x200B;**套用**&#x200B;和&#x200B;**儲存**
...
如前所述，XDM和即時客戶個人檔案的力量是能夠將個人的各種片段及其行為組合在一起。  此彙總稱為客戶的「聯合檢視」。  在下列步驟中，您會預覽針對即時客戶設定檔設定的每個XDM類別中此聯合的外觀
