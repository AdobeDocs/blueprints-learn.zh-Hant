...
XDM個別設定檔類別型結構描述可讓您收集與設定檔結合之個人的相關屬性。 類別本身包含無法編輯的欄位，例如&#x200B;*modifiedByBatchID*、*PersonID*&#x200B;等。
...
Adobe Experience Platform中有許多欄位群組是以標準XDM的形式存在，可供您新增至結構描述並進行自訂。
...
1. 搜尋&#x200B;**人口統計詳細資料**，或瀏覽清單來尋找它。
   ...
   - 當您找到欄位群組時，請按一下欄位群組右側的放大鏡來檢視其結構。  此步驟是預覽您要新增至結構描述的內容而不新增它的有用方式。
   - 檢閱完成時關閉預覽
...
完成後，您的結構描述看起來會如下所示。 請務必按一下「**儲存**」按鈕並儲存您的工作！
...
若要從任何Adobe標準欄位群組中移除欄位，請使用&#x200B;**管理相關欄位**&#x200B;選項。 管理相關欄位可讓您從結構描述中移除標準欄位，因此僅保留您需要的欄位。
...
1. 完成後，您應該會在結構描述中看到人員物件，如下所示。 若要儲存您的結構描述，如果一切正常，請按一下&#x200B;**儲存**&#x200B;按鈕。
...
完成後，您的最終結構描述現在看起來會像這樣。 繼續之前，請務必按一下&#x200B;**儲存**。
