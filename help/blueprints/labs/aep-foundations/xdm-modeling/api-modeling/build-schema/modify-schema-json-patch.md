...
假設在建立結構描述後，您需要將額外的欄位新增到名為`planDescription`的`plan`物件。 這可能是因為您在建立結構時忘記新增它，或因為它是幾個月後才會收到的請求。 若要執行此工作，請執行以新欄位更新結構描述的`PATCH`作業。

請前往下列連結，以進一步瞭解JSON PATCH。 在本實驗中，假設您已大致瞭解其運作方式。
...
>請記住以下要點：
>
>- 結構描述由一個類別和一個或多個欄位群組組成
>- 您必須先將新欄位新增到欄位群組，才能將其新增到結構描述。 此限制可確保跨使用該欄位群組的任何結構描述之欄位的可重複使用性。



若要將新欄位新增到結構描述，您必須依序執行下列操作。 此程式是您在下列實驗步驟中所執行的操作。

- 識別您要新增屬性的欄位群組
- 建構JSON PATCH呼叫以更新欄位群組
- 執行JSON PATCH呼叫以更新欄位群組（結構描述會繼承）
...
1. 按一下`Send`按鈕以執行要求
   ...
   >[!NOTE]
   >
   >請記得您在自訂欄位群組中建立`plan`物件。 XDM結構描述登入中自訂建立的物件稱為「租使用者」，因此是使用`/schemaregistry/tenant/mixins/`路徑的API呼叫。



1. 在您先前建立且標題為`Customer Account Details - Sandbox <your number here> `的自訂欄位群組的結構描述ID回應搜尋中

1. 複製`$meta:altId`並儲存到您下一步所需的安全位置

![在API回應中尋找自訂客戶帳戶詳細資料欄位群組](assets/modify-schema-json-patch-search-field-group-response.jpeg "搜尋回應中的客戶帳戶詳細資料欄位群組")

>[!CAUTION]
>
>請確定您選取要複製的正確欄位群組！ 請勿使用名稱相似且名為`dep: Customer Account Details`的欄位群組

>[!WARNING]
>
>您需要`$meta:altId`才能進行未來的實驗室步驟，因此請先將它儲存於其他位置再繼續
...
- **路徑** ->這是您要建立、更新或刪除的路徑（也就是新欄位位置的JSON指標）
...
您會在欄位群組中看到`200 OK`回應和`planDescription`欄位，如下所示：
