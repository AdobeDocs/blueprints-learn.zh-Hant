...
1. 將`Postman API Collection File`的URL貼到覆蓋圖上的匯入模組文字方塊中。  這應該會觸發自動匯入
...
您現在應該會在左側邊欄的`Collections`標籤下看到名稱為`AEP Foundations Bootcamp`的集合已填入
...
您匯入的API集合包含您在整個Bootcamp中Labs所需的所有必要API呼叫。  每個實驗室都會分組到具有自己一組API的特定資料夾中。  當您本週在Labs中工作時，請注意這一點。
