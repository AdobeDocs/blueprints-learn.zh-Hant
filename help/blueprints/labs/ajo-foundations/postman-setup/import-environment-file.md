...
在此頁面中，您會匯入Postman環境檔案。  此檔案包含一些全域變數，這些變數用於您在整個bootcamp中的其他Labs進行的各種API呼叫中。
...
2. 在本機電腦上啟動Postman。
3. 如有必要，請切換到您用於這些Labs的Workspace，然後按一下&#x200B;**匯入**&#x200B;按鈕。

   ![Postman開始匯入](assets/import-environment-file-click-import-button.png)

4. 將&#x200B;**AJO Bootcamp.postman\_environment.json**&#x200B;檔案的本機URL貼入匯入模組文字方塊，或將它拖放到匯入對話方塊中。  此動作會觸發自動匯入
...
Postman是專為測試及與API互動而設計。 不過，本實驗會使用它來模擬來自瀏覽器的AEP Web SDK點選，或用於伺服器端的即時資料收集呼叫。 雖然這些要求從技術上講屬於API呼叫，但並非典型API呼叫需要標頭中的授權權杖等專案。 這些Labs中的環境變數主要用於URL路徑中的變數（有一個用於標頭）。
...
3. 現在跳過DATASTREAM\_CONFIG值。 您可在稍後的實驗室中建立資料串流設定。
...
5. 您現在需要儲存環境變數；不過Postman UI中沒有儲存按鈕。 使用Windows或Mac快速鍵進行儲存（例如Windows上的Ctrl+s）。 當您在Postman UI的右下角看到&#x200B;**已儲存的變更**&#x200B;訊息時，就表示您的變更已儲存：
