...
透過個人化生日訊息和獨家優惠，鎖定生日上的來賓。 生日行銷活動會認可個人里程碑，並鼓勵慶祝預訂或造訪，藉此加強賓客關係。

### 企業影響

生日訊息可建立情緒化的接觸點，區分品牌並促進遞增預訂，因為當有吸引人的個人化優惠方案出現時，訪客更可能在他們的生日臨近時計畫度假或就餐體驗。
...
- 生日日期必須擷取到訪客設定檔並進行驗證，以避免在不正確的日期傳送訊息。
- 優惠方案應具有定義的有效性時段（例如生日月份），為來賓提供合理的時間來計畫和預訂住宿或體驗。
- 檔案上沒有生日的來賓應從行銷活動中排除，而不是傳送一般訊息。
- 選件個人化應考量先前的預訂偏好設定（目的地、屬性型別、房間類別）以呈現相關建議。

## 目的地促銷活動

在持續進行旅遊目的地促銷活動期間，鎖定要預訂的訪客。 目的地促銷活動可透過為旅客提供符合其興趣的促銷目的地及時優惠來連結旅客，進而推動預訂。

### 企業影響

目標目的地促銷活動會根據過往的旅行記錄和所述的偏好設定，觸及最有可能感興趣的旅行者，藉此改善預訂轉換、減少促銷活動浪費並提高促銷活動ROI。

### 實施方式

使用[批次傳出訊息啟用](/help/blueprints/use-case-patterns/campaign-management-orchestration/batch-outbound-message-activation.md)模式，在作用中目的地行銷活動期間將促銷訊息傳送至合格的對象區段。 排程的個人化促銷訊息批次需要在限定時間的行銷活動期間觸及定義的對象時，此為正確的模式。

### 技術考量

- 必須管理促銷開始與結束日期，以確保訊息只會在有效促銷期間傳送。
- 對象細分應運用過去的預訂歷史記錄、瀏覽行為以及目的地相關性，以定位最有可能與促銷目的地互動的訪客。
- 已針對促銷目的地和旅行日期預訂的訪客應從贏取訊息中隱藏。
