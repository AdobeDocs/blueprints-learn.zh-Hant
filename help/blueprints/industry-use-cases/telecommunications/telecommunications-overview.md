...
根據客戶位置和使用模式，使用個人化優惠和權益來鎖定符合升級資格的客戶。 隨著新服務範圍不斷擴大，透過相關訊息服務觸及新服務區域的訂閱者，可加快採用率並提升網路使用率。

### 企業影響

目標明確的升級促銷活動可提升合格訂閱者的採用率，支援網路投資回報及競爭差異化。

### 實施方式

使用[批次傳出訊息啟用](/help/blueprints/use-case-patterns/campaign-management-orchestration/batch-outbound-message-activation.md)模式，根據涵蓋範圍可用性、裝置相容性和計畫適用性來劃分訂閱者，然後提供個人化升級行銷活動，強調與每個訂閱者的使用情況設定檔最相關的權益。 當對象已預先定義且規模較大、傳送時間已排程而非事件導向，且不需要即時分支或決策時，就是正確的模式；活動可以根據涵蓋範圍轉出時間表預先完全規劃。

### 技術考量

- 整合網路涵蓋範圍圖，以準確識別使用中服務的區域的訂閱者，並避免在尚未提供涵蓋範圍的區域推廣升級。
- 連線裝置相容性資料，以判斷哪些訂閱者需要新裝置，而哪些訂閱者已有具備功能的硬體。
