...
description: 使用Adobe [!DNL Experience Platform] (AEM)對資料湖中存在的資料執行探索性查詢和分析。
...
資料分析和智慧包括[!DNL Experience Platform]內部的功能，可針對資料湖中存在的資料執行探索性查詢和分析。

[!DNL Experience Platform]的[!UICONTROL 查詢服務]允許對資料執行SQL查詢。

[!DNL Experience Platform]允許與協力廠商SQL使用者端、介面及Business Intelligence(BI)工具的連線，以使用[!DNL PostgreSQL]通訊協定直接連線、存取及查詢[!DNL Experience Platform]內的資料。
