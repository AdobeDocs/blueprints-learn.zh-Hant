...
5. **支援的業務目標** — `/help/blueprints/business-objectives/`下現有集合中的一或多個業務目標。 每個檔案夾都應包含目標名稱、類別子檔案夾和檔案名稱。 在產生內容之前，請確認參考的檔案存在。

6. **戰術使用案例範例** — 6至10個專案符號情境，說明如何在不同業務內容中套用此模式。 每個案例都應該有粗體案例名稱及後續說明。

7. **KPI** — 包含三欄的表格：KPI （名稱）、說明（測量專案）、測量（公式或方法）。

8. **參考連結** — 參考主要Experience League檔案的連結，涵蓋使用案例模式的應用程式和功能。
...
如果使用者未提供選擇性料號，則根據模式分類、解決方案及執行計畫產生合理的預設值。
...
1. **商業目標連結** — 模式檔案中的每個商業目標連結都指向`/help/blueprints/business-objectives/`下的現有檔案。 使用工作區搜尋或檔案讀取來確認每個目標檔案都存在。
